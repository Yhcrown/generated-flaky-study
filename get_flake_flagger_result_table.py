...
def analyze_positive_result(write_dir):
    ...
    group_list = ['random_group','date_group',"sleep_group","time_group","socket_group", "locale_group"]
    ...
    with open(write_dir, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        for project, info in project_dict.items():
            ...
            if 'config_only_locale_fix' in info:
                row.append(len (  (info['chaos_failed_tests'] - info['flaky_tests']) - info['config_only_locale_fix'] ))
            else:
                print(project)
            ...
            if len(info['total_tests']) != 0:
                ...
                sum_tp_without_zone +=len (  (info['chaos_failed_tests'] - info['flaky_tests']) - info['config_only_locale_fix'] )
